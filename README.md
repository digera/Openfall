...
**Bunny hopping.** A jump pressed within 50 ms (three ticks) either side of the moment your feet touch down is a hop: your feet never grip and all of your speed carries into the next jump. A press just before landing is buffered, so it still counts if the key has already come back up, and your feet coast for the late half of the window so there is something left to carry. Any other jump leaves at run speed, including holding Space through the landing. Holding Space bounces; timing it keeps the speed. The windows are `HOP_EARLY_TICKS` / `HOP_LATE_TICKS` in `src/simulation.odin`.
