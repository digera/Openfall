...
When a wisp dies, it inflates like a balloon over 0.40 seconds (the robe radii scale up with an ease-out curve, brightening as it swells), then pops: the robe and motes vanish and a brief bright flash (0.12 seconds) blooms at the body's position in the team's colour before fading. The wisp stays hidden until respawn. The death animation is replicated: every client who can see the wisp sees the same inflate-and-pop, driven by the `dead` flag in the snapshot. First-person death (when the local player dies) shows the same flash through the `fx2.y` dead flag rather than rendering a local robe.
