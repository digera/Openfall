...
layout(binding=1) uniform fs_params {
    ...
    vec4 pylon_node_meta[7]; // x max_count, y spiral_radius, z stack_step, w alive bitmask (as f32)
...
// spiral of node spheres at FIXED positions (by array index, not HP rank). Analytic
// hits, no occupancy atlas. Chip damage is a scar (pylon_wounds); dead nodes are
// omitted (visible holes), core matches live column. Must stay in lockstep with
// src/tower_nodes.odin.
...
// Node position from ARRAY INDEX (stable), not rank (HP-sorted).
...
// Clip a ray to a Z-aligned cylinder in local space. Twin of tower_bound_clip.
...
bool pylon_trace_local(vec3 ro, vec3 rd, float core_h,
                       int max_count, float spiral_r, float step, float node_r,
                       uint alive_mask, float outer_r, float max_t,
    ...
    // Iterate FIXED slots by array index, skip dead nodes.
    // Geometry is stable: damage and death do not move positions.
    int nmax = min(max_count, PYLON_NODE_MAX);
...
// Nearest pylon along the ray. `local` and `part` come back so the material
// stage can tell core from node without redoing the transform.
bool pylon_trace(vec3 ro, vec3 rd, float tmax, out float t, out vec3 n, out int idx, out vec3 local, out float part) {
    bool hit = false;
    t = tmax;
    n = vec3(0.0, 0.0, 1.0);
    idx = 0;
    ...
    for (int i = 0; i < NPYLON; i++) {
        ...
        int max_count = int(meta.x + 0.5);
        float spiral_r = meta.y;
        float step = meta.z;
        uint alive_mask = uint(meta.w);
        ...
        vec3 ln;
        float lp_part;
        if (!pylon_trace_local(lo, ld, sh.x, max_count, spiral_r, step, b.y, alive_mask, b.z, t, lt, ln, lp_part)) continue;
        vec3 lp = lo + ld * lt;
        t = lt;
        n = unrot_z(ln, s, c);
        idx = i;
...
void main() {
    ...
    float pyt;
    vec3 pyn;
    if (pylon_trace(ro, rd, best, pyt, pyn, py_idx, py_local, py_part)) {
