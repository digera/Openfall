...
play_death :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	frames := int(math.ceil_f64(f64(sr) * 1.083))
	out := make([]f32, frames * 2)
	defer delete(out)

	unmake: ma.waveform
	unmake_f0 := f32(360) * (f32(0.96) + rand.float32() * f32(0.08))
	unmake_f1 := max(f32(0.001), f32(48) * (f32(0.96) + rand.float32() * f32(0.08)))
	unmake_cfg := ma.waveform_config_init(.f32, 1, sr, .sawtooth, 1, f64(unmake_f0))
	if ma.waveform_init(&unmake_cfg, &unmake) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&unmake)
	unmake_buf := make([]f32, frames)
	defer delete(unmake_buf)
	read_waveform_ramp(&unmake, unmake_buf, sr, unmake_f0, unmake_f1, 0, f32(0.42), .exp)

	ash: ma.noise
	ash_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&ash_cfg, nil, &ash) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&ash, nil)
	ash_buf := make([]f32, frames)
	defer delete(ash_buf)
	read_noise_window(&ash, ash_buf, sr, 0, f32(0.32))

	body: ma.waveform
	body_f0 := f32(92) * (f32(0.95) + rand.float32() * f32(0.1))
	body_f1 := max(f32(0.001), f32(28) * (f32(0.95) + rand.float32() * f32(0.1)))
	...
	read_waveform_ramp(&body, body_buf, sr, body_f0, body_f1, 0, f32(0.48), .exp)

	unmakeAmp_peak := max(f32(0.001), f32(0.16) * (f32(0.96) + rand.float32() * f32(0.08)))
	unmakeAmp_buf := make([]f32, frames)
	defer delete(unmakeAmp_buf)
	gain_env(unmake_buf, unmakeAmp_buf, sr, unmakeAmp_peak, 0, f32(0.42))

	ashBp: ma.biquad
	ashBp_f0 := f32(880) * (f32(0.94) + rand.float32() * f32(0.12))
	ashBp_f1 := max(f32(0.001), f32(160) * (f32(0.94) + rand.float32() * f32(0.12)))
	if !biquad_init_rbj(&ashBp, sr, .bandpass, ashBp_f0, f32(0.9)) {
		return
	}
	defer ma.biquad_uninit(&ashBp, nil)
	ashBp_buf := make([]f32, frames)
	defer delete(ashBp_buf)
	biquad_sweep(&ashBp, ash_buf, ashBp_buf, sr, .bandpass, f32(0.9), ashBp_f0, ashBp_f1, f32(0.26))

	ashAmp_peak := max(f32(0.001), f32(0.12) * (f32(0.95) + rand.float32() * f32(0.1)))
	ashAmp_buf := make([]f32, frames)
	defer delete(ashAmp_buf)
	gain_env(ashBp_buf, ashAmp_buf, sr, ashAmp_peak, 0, f32(0.32))

	bodyAmp_peak := max(f32(0.001), f32(0.22) * (f32(0.96) + rand.float32() * f32(0.08)))
	bodyAmp_buf := make([]f32, frames)
	defer delete(bodyAmp_buf)
	gain_env(body_buf, bodyAmp_buf, sr, bodyAmp_peak, 0, f32(0.48))

	wash_buf := make([]f32, frames)
	defer delete(wash_buf)
	delay_line(unmakeAmp_buf, wash_buf, sr, f32(0.22), f32(0.48), f32(0.55))

	add_mono_to_stereo(out, ashAmp_buf)
	add_mono_to_stereo(out, bodyAmp_buf)
	add_mono_to_stereo(out, wash_buf)
