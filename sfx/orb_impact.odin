...
play_orb_impact :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	frames := int(math.ceil_f64(f64(sr) * 0.584))
	out := make([]f32, frames * 2)
	defer delete(out)

	blast: ma.noise
	blast_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&blast_cfg, nil, &blast) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&blast, nil)
	blast_buf := make([]f32, frames)
	defer delete(blast_buf)
	read_noise_window(&blast, blast_buf, sr, 0, f32(0.09))

	boom: ma.waveform
	boom_f0 := f32(68) * (f32(0.95) + rand.float32() * f32(0.1))
	boom_f1 := max(f32(0.001), f32(26) * (f32(0.95) + rand.float32() * f32(0.1)))
	boom_cfg := ma.waveform_config_init(.f32, 1, sr, .sine, 1, f64(boom_f0))
	if ma.waveform_init(&boom_cfg, &boom) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&boom)
	boom_buf := make([]f32, frames)
	defer delete(boom_buf)
	read_waveform_ramp(&boom, boom_buf, sr, boom_f0, boom_f1, 0, f32(0.3), .exp)

	edge: ma.waveform
	edge_f0 := f32(170) * (f32(0.9) + rand.float32() * f32(0.2))
	edge_f1 := max(f32(0.001), f32(48) * (f32(0.9) + rand.float32() * f32(0.2)))
	...
	read_waveform_ramp(&edge, edge_buf, sr, edge_f0, edge_f1, 0, f32(0.04), .exp)

	blastBp: ma.biquad
	blastBp_f0 := f32(920) * (f32(0.92) + rand.float32() * f32(0.16))
	blastBp_f1 := max(f32(0.001), f32(140) * (f32(0.92) + rand.float32() * f32(0.16)))
	if !biquad_init_rbj(&blastBp, sr, .bandpass, blastBp_f0, f32(0.85)) {
		return
	}
	defer ma.biquad_uninit(&blastBp, nil)
	blastBp_buf := make([]f32, frames)
	defer delete(blastBp_buf)
	biquad_sweep(&blastBp, blast_buf, blastBp_buf, sr, .bandpass, f32(0.85), blastBp_f0, blastBp_f1, f32(0.07))

	smash_buf := make([]f32, frames)
	defer delete(smash_buf)
	waveshape(blastBp_buf, smash_buf, f32(14))

	blastAmp_peak := max(f32(0.001), f32(0.26) * (f32(0.94) + rand.float32() * f32(0.12)))
	blastAmp_buf := make([]f32, frames)
	defer delete(blastAmp_buf)
	gain_env(smash_buf, blastAmp_buf, sr, blastAmp_peak, 0, f32(0.09))

	boomAmp_peak := max(f32(0.001), f32(0.34) * (f32(0.96) + rand.float32() * f32(0.08)))
	boomAmp_buf := make([]f32, frames)
	defer delete(boomAmp_buf)
	gain_env(boom_buf, boomAmp_buf, sr, boomAmp_peak, 0, f32(0.3))

	edgeAmp_peak := max(f32(0.001), f32(0.07) * (f32(0.9) + rand.float32() * f32(0.2)))
	edgeAmp_buf := make([]f32, frames)
	defer delete(edgeAmp_buf)
	gain_env(edge_buf, edgeAmp_buf, sr, edgeAmp_peak, 0, f32(0.04))

	hall_buf := make([]f32, frames)
	defer delete(hall_buf)
	delay_line(blastAmp_buf, hall_buf, sr, f32(0.12), f32(0.32), f32(0.4))

	add_mono_to_stereo(out, boomAmp_buf)
	add_mono_to_stereo(out, edgeAmp_buf)
	add_mono_to_stereo(out, hall_buf)
