...
play_blink_charge :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	fold: ma.waveform
	fold_f0 := f32(480) * (f32(0.96) + rand.float32() * f32(0.08))
	fold_f1 := f32(760) * (f32(0.96) + rand.float32() * f32(0.08))
	fold_cfg := ma.waveform_config_init(.f32, 1, sr, .sine, 1, f64(fold_f0))
	if ma.waveform_init(&fold_cfg, &fold) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&fold)
	fold_buf := make([]f32, frames)
	defer delete(fold_buf)
	read_waveform_ramp(&fold, fold_buf, sr, fold_f0, fold_f1, 0, f32(0.1), .lin)

	veil: ma.noise
	veil_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&veil_cfg, nil, &veil) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&veil, nil)
	veil_buf := make([]f32, frames)
	defer delete(veil_buf)
	read_noise_window(&veil, veil_buf, sr, 0, f32(0.08))

	foldAmp_peak := max(f32(0.001), f32(0.11) * (f32(0.95) + rand.float32() * f32(0.1)))
	foldAmp_buf := make([]f32, frames)
	defer delete(foldAmp_buf)
	gain_env(fold_buf, foldAmp_buf, sr, foldAmp_peak, 0, f32(0.1))

	veilBp: ma.biquad
	veilBp_f0 := f32(1800) * (f32(0.94) + rand.float32() * f32(0.12))
	veilBp_f1 := max(f32(0.001), f32(4200) * (f32(0.94) + rand.float32() * f32(0.12)))
	if !biquad_init_rbj(&veilBp, sr, .bandpass, veilBp_f0, f32(2.8)) {
		return
	}
	defer ma.biquad_uninit(&veilBp, nil)
	veilBp_buf := make([]f32, frames)
	defer delete(veilBp_buf)
	biquad_sweep(&veilBp, veil_buf, veilBp_buf, sr, .bandpass, f32(2.8), veilBp_f0, veilBp_f1, f32(0.07))

	veilAmp_peak := max(f32(0.001), f32(0.06) * (f32(0.92) + rand.float32() * f32(0.16)))
	veilAmp_buf := make([]f32, frames)
	defer delete(veilAmp_buf)
	gain_env(veilBp_buf, veilAmp_buf, sr, veilAmp_peak, 0, f32(0.08))

	shift_buf := make([]f32, frames * 2)
	defer delete(shift_buf)
	pan_to_stereo(veilAmp_buf, shift_buf, f32(-0.28))

	add_mono_to_stereo(out, foldAmp_buf)
	add_stereo(out, shift_buf)
