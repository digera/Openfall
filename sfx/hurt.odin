...
play_hurt :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	flesh: ma.noise
	flesh_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&flesh_cfg, nil, &flesh) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&flesh, nil)
	flesh_buf := make([]f32, frames)
	defer delete(flesh_buf)
	read_noise_window(&flesh, flesh_buf, sr, 0, f32(0.08))

	gut: ma.waveform
	gut_f0 := f32(210) * (f32(0.94) + rand.float32() * f32(0.12))
	gut_f1 := max(f32(0.001), f32(68) * (f32(0.94) + rand.float32() * f32(0.12)))
	gut_cfg := ma.waveform_config_init(.f32, 1, sr, .sine, 1, f64(gut_f0))
	if ma.waveform_init(&gut_cfg, &gut) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&gut)
	gut_buf := make([]f32, frames)
	defer delete(gut_buf)
	read_waveform_ramp(&gut, gut_buf, sr, gut_f0, gut_f1, 0, f32(0.13), .exp)

	fleshLp: ma.biquad
	fleshLp_f0 := f32(760) * (f32(0.92) + rand.float32() * f32(0.16))
	fleshLp_f1 := max(f32(0.001), f32(280) * (f32(0.92) + rand.float32() * f32(0.16)))
	if !biquad_init_rbj(&fleshLp, sr, .lowpass, fleshLp_f0, f32(0.8)) {
		return
	}
	defer ma.biquad_uninit(&fleshLp, nil)
	fleshLp_buf := make([]f32, frames)
	defer delete(fleshLp_buf)
	biquad_sweep(&fleshLp, flesh_buf, fleshLp_buf, sr, .lowpass, f32(0.8), fleshLp_f0, fleshLp_f1, f32(0.06))

	bruise_buf := make([]f32, frames)
	defer delete(bruise_buf)
	waveshape(fleshLp_buf, bruise_buf, f32(4))

	fleshAmp_peak := max(f32(0.001), f32(0.18) * (f32(0.93) + rand.float32() * f32(0.14)))
	fleshAmp_buf := make([]f32, frames)
	defer delete(fleshAmp_buf)
	gain_env(bruise_buf, fleshAmp_buf, sr, fleshAmp_peak, 0, f32(0.08))

	gutAmp_peak := max(f32(0.001), f32(0.2) * (f32(0.95) + rand.float32() * f32(0.1)))
	gutAmp_buf := make([]f32, frames)
	defer delete(gutAmp_buf)
	gain_env(gut_buf, gutAmp_buf, sr, gutAmp_peak, 0, f32(0.13))

	add_mono_to_stereo(out, fleshAmp_buf)
	add_mono_to_stereo(out, gutAmp_buf)
