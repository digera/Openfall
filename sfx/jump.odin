...
play_jump :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	lift: ma.waveform
	lift_f0 := f32(210) * (f32(0.94) + rand.float32() * f32(0.12))
	lift_f1 := max(f32(0.001), f32(140) * (f32(0.94) + rand.float32() * f32(0.12)))
	lift_cfg := ma.waveform_config_init(.f32, 1, sr, .sine, 1, f64(lift_f0))
	if ma.waveform_init(&lift_cfg, &lift) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&lift)
	lift_buf := make([]f32, frames)
	defer delete(lift_buf)
	read_waveform_ramp(&lift, lift_buf, sr, lift_f0, lift_f1, 0, f32(0.045), .exp)
	...
	read_noise_window(&air, air_buf, sr, 0, f32(0.02))

	liftAmp_peak := max(f32(0.001), f32(0.1) * (f32(0.94) + rand.float32() * f32(0.12)))
	liftAmp_buf := make([]f32, frames)
	defer delete(liftAmp_buf)
	gain_env(lift_buf, liftAmp_buf, sr, liftAmp_peak, 0, f32(0.045))
	...
	airAmp_peak := max(f32(0.001), f32(0.04) * (f32(0.9) + rand.float32() * f32(0.2)))
	airAmp_buf := make([]f32, frames)
	defer delete(airAmp_buf)
	gain_env(airHp_buf, airAmp_buf, sr, airAmp_peak, 0, f32(0.02))

	add_mono_to_stereo(out, liftAmp_buf)
