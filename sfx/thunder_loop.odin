...
play_thunder_loop :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	buzz: ma.waveform
	buzz_f0 := f32(92) * (f32(0.96) + rand.float32() * f32(0.08))
	buzz_cfg := ma.waveform_config_init(.f32, 1, sr, .sawtooth, 1, f64(buzz_f0))
	if ma.waveform_init(&buzz_cfg, &buzz) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&buzz)
	buzz_buf := make([]f32, frames)
	defer delete(buzz_buf)
	read_waveform_ramp(&buzz, buzz_buf, sr, buzz_f0, buzz_f0, 0, f32(0.09), .exp)

	arc: ma.noise
	arc_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&arc_cfg, nil, &arc) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&arc, nil)
	arc_buf := make([]f32, frames)
	defer delete(arc_buf)
	read_noise_window(&arc, arc_buf, sr, 0, f32(0.07))

	hum: ma.waveform
	hum_f0 := f32(46) * (f32(0.95) + rand.float32() * f32(0.1))
	hum_cfg := ma.waveform_config_init(.f32, 1, sr, .square, 1, f64(hum_f0))
	if ma.waveform_init(&hum_cfg, &hum) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&hum)
	hum_buf := make([]f32, frames)
	defer delete(hum_buf)
	read_waveform_ramp(&hum, hum_buf, sr, hum_f0, hum_f0, 0, f32(0.09), .exp)

	buzzAmp_peak := max(f32(0.001), f32(0.1) * (f32(0.96) + rand.float32() * f32(0.08)))
	buzzAmp_buf := make([]f32, frames)
	defer delete(buzzAmp_buf)
	gain_env(buzz_buf, buzzAmp_buf, sr, buzzAmp_peak, 0, f32(0.09))

	arcBp: ma.biquad
	arcBp_f0 := f32(2400) * (f32(0.9) + rand.float32() * f32(0.2))
	arcBp_f1 := max(f32(0.001), f32(1600) * (f32(0.9) + rand.float32() * f32(0.2)))
	if !biquad_init_rbj(&arcBp, sr, .bandpass, arcBp_f0, f32(1.6)) {
		return
	}
	defer ma.biquad_uninit(&arcBp, nil)
	arcBp_buf := make([]f32, frames)
	defer delete(arcBp_buf)
	biquad_sweep(&arcBp, arc_buf, arcBp_buf, sr, .bandpass, f32(1.6), arcBp_f0, arcBp_f1, f32(0.05))

	bite_buf := make([]f32, frames)
	defer delete(bite_buf)
	waveshape(arcBp_buf, bite_buf, f32(3))

	arcAmp_peak := max(f32(0.001), f32(0.11) * (f32(0.92) + rand.float32() * f32(0.16)))
	arcAmp_buf := make([]f32, frames)
	defer delete(arcAmp_buf)
	gain_env(bite_buf, arcAmp_buf, sr, arcAmp_peak, 0, f32(0.07))

	humAmp_peak := max(f32(0.001), f32(0.05) * (f32(0.94) + rand.float32() * f32(0.12)))
	humAmp_buf := make([]f32, frames)
	defer delete(humAmp_buf)
	gain_env(hum_buf, humAmp_buf, sr, humAmp_peak, 0, f32(0.09))

	add_mono_to_stereo(out, buzzAmp_buf)
	add_mono_to_stereo(out, arcAmp_buf)
	add_mono_to_stereo(out, humAmp_buf)
