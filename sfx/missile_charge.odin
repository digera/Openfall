...
play_missile_charge :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	frames := int(math.ceil_f64(f64(sr) * 0.15))
	out := make([]f32, frames * 2)
	defer delete(out)

	shimmer: ma.waveform
	shimmer_f0 := f32(420) * (f32(0.95) + rand.float32() * f32(0.1))
	shimmer_f1 := f32(720) * (f32(0.95) + rand.float32() * f32(0.1))
	shimmer_cfg := ma.waveform_config_init(.f32, 1, sr, .triangle, 1, f64(shimmer_f0))
	if ma.waveform_init(&shimmer_cfg, &shimmer) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&shimmer)
	shimmer_buf := make([]f32, frames)
	defer delete(shimmer_buf)
	read_waveform_ramp(&shimmer, shimmer_buf, sr, shimmer_f0, shimmer_f1, 0, f32(0.13), .lin)

	glint: ma.noise
	glint_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&glint_cfg, nil, &glint) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&glint, nil)
	glint_buf := make([]f32, frames)
	defer delete(glint_buf)
	read_noise_window(&glint, glint_buf, sr, f32(0.02), f32(0.07))

	shimmerAmp_peak := max(f32(0.001), f32(0.1) * (f32(0.95) + rand.float32() * f32(0.1)))
	shimmerAmp_buf := make([]f32, frames)
	defer delete(shimmerAmp_buf)
	gain_env(shimmer_buf, shimmerAmp_buf, sr, shimmerAmp_peak, 0, f32(0.13))

	glintBp: ma.biquad
	glintBp_f0 := f32(4200) * (f32(0.94) + rand.float32() * f32(0.12))
	glintBp_f1 := max(f32(0.001), f32(6400) * (f32(0.94) + rand.float32() * f32(0.12)))
	if !biquad_init_rbj(&glintBp, sr, .bandpass, glintBp_f0, f32(4.2)) {
		return
	}
	defer ma.biquad_uninit(&glintBp, nil)
	glintBp_buf := make([]f32, frames)
	defer delete(glintBp_buf)
	biquad_sweep(&glintBp, glint_buf, glintBp_buf, sr, .bandpass, f32(4.2), glintBp_f0, glintBp_f1, f32(0.04))

	glintAmp_peak := max(f32(0.001), f32(0.05) * (f32(0.92) + rand.float32() * f32(0.16)))
	glintAmp_buf := make([]f32, frames)
	defer delete(glintAmp_buf)
	gain_env(glintBp_buf, glintAmp_buf, sr, glintAmp_peak, f32(0.02), f32(0.07))

	add_mono_to_stereo(out, shimmerAmp_buf)
	add_mono_to_stereo(out, glintAmp_buf)
