...
hud_playing :: proc(gc: ^Game_Client, cols, rows: f32) {
	...
	if world.have_game_state && Match_State(gs.match_state) == .Waiting {
		...
		hud_center_text(cols, 7, "bring the golden tower down, then build it back: finish the tower to win")
	}
	...
	if world.have_game_state && Match_State(gs.match_state) == .Active {
		if gs.centre_open {
			sdtx.color3f(0.95, 0.88, 0.55)
			hud_center_text(cols, 6, "the centre is open - your waves are rebuilding it, finish the tower to win")
