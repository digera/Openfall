...
WORLD_PLAZA_HALF      :: f32(16)      // center plaza unchanged
WORLD_LANE_R0         :: f32(14)      // lane starts (overlaps plaza)
WORLD_LANE_R1         :: f32(100)     // lane ends (overlaps base) - DOUBLED
WORLD_LANE_HALF_W     :: f32(4.5)
WORLD_BASE_R          :: f32(110)     // base centers - DOUBLED + 10
WORLD_BASE_HALF       :: f32(11)
WORLD_SPAWN_R         :: f32(114)     // spawn points - DOUBLED + 4
WORLD_LANE_OBELISK_R  :: f32(24)      // near-lane Obelisk (unchanged)
WORLD_LANE_OBELISK_FAR_R :: f32(70)   // far-lane Obelisk (new, ~70% of lane)
WORLD_EXTENT          :: f32(125)     // rough outer radius (fog / culling) - DOUBLED - 25
...
world_map_init :: proc "contextless" () {
	...
	// Lane cover: four staggered crates per lane for extended lanes.
	for team in TEAMS {
		a := team_angle(team)
		d := team_dir(team)
		side := vec3{-d.y, d.x, 0}
		// Near cover (original positions, adjusted for longer lanes)
		...
		// Far cover (new, for extended lanes)
...
obelisk_position :: proc(index: int) -> vec3 {
	...
	// Near-lane Obelisks: indices 1-3
	if index >= 1 && index <= 3 {
