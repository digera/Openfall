...
echo "Starting server with default bots (1 per team)..."
...
if [ -z "$REMOTE_COUNT" ]; then
    ...
    exit 1
elif [ "$REMOTE_COUNT" -lt "3" ]; then
    echo "❌ FAIL: Expected at least 3 remotes (1 bot per team), got $REMOTE_COUNT"
    exit 1
else
    echo "✓ Client sees $REMOTE_COUNT remote entities (bots)"
fi

# Check server entity count ([Stats] ... | entities N | ...)
SERVER_ENTITIES=$(grep -oP "entities \K[0-9]+" /tmp/remote_test_server.log | tail -1)
